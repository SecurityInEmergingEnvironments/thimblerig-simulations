...
class Simulation(object):
    ...
    def percent_change_over_baseline(self, baseline_callable, comparative_series: pd.Series) -> pd.Series:
        percent_change = {}
        for index, comparable_value in comparative_series.items():
            try:
                baseline_value = baseline_callable(index)
                difference = baseline_value - comparable_value
                percent_change[index] = (float(difference) / baseline_value) * 100
            except ValueError:
                percent_change[index] = np.nan

        return pd.Series(percent_change).interpolate()
    ...
    def draw(self):
        ...
        for index, ttl_percents in enumerate(self.ttl_percents):
            ...
            highest_ttl = max(ttl_percents)
            baseline_impact = None
            # Iterate in descending order
            for ttl_percent in sorted(ttl_percents, reverse=True):
                ...
                impact_series: pd.Series = mean_columns.cumsum().map(
                        lambda usable_time: compute_impact(usable_time * RECORDS_PER_UNIT_TIME))
                if ttl_percent == highest_ttl:
                    baseline_impact = scipy.interpolate.interp1d(impact_series.index, impact_series.values)
                else:
                    percent_change = self.percent_change_over_baseline(baseline_impact, impact_series)
                    ax.plot(percent_change, label="TTL {}% of Sim time".format(ttl_percent), linewidth=3)
